...
std::string elgamal_encrypt(BIGNUM* g, BIGNUM* g_hat, BIGNUM* q, BIGNUM* p, BIGNUM* y, const std::string& m) {
    ...
    // Hash the dec string using SHA-256
...
void hash4(const BIGNUM* g1, const BIGNUM* g2, const BIGNUM* g3, const BIGNUM* q, BN_CTX* xd, BIGNUM* result){
    char* g1_dec = BN_bn2dec(g1);
    char* g2_dec = BN_bn2dec(g2);
    char* g3_dec = BN_bn2dec(g3);

    // Concatenate the input values as a single string
    std::string input_str =  std::string(g1_dec) + g2_dec + g3_dec;
...
//std::string partialDecrypt(std::string cipher,)